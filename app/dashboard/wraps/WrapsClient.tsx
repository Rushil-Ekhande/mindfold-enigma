...
export default function WrapsClient({ entries }: WrapsClientProps) {
    ...
    const averages = useMemo(() => {
        const result: Record<string, { avg: number; count: number; trend: number }> =
            {};
    ...
    const overallScore = useMemo(() => {
        ...
        return Math.round(
            (mh + hap + acc + (100 - stress) + (100 - burnout)) / 5
        );
