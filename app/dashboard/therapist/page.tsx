...
export default function TherapistPage() {
  ...
  const [detailTherapist, setDetailTherapist] = useState<TherapistData | null>(
    null,
  );
  ...
  if (hasTherapist && relationship) {
    async function unhireTherapist() {
      ...
      if (result.success) {
        ...
        alert(
          "Failed to end relationship: " + (result.error || "Unknown error"),
        );
      }
    }
                  ...
                  {detailTherapist.therapist_services.map((service) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() =>
                            subscribeToService(detailTherapist.id, service.id)
                          }
...
function ChatTab({
  ...
  const fetchMessages = useCallback(async () => {
    const res = await fetch(`/api/messages?relationship_id=${relationshipId}`, {
      cache: "no-store",
    });
    const data = await res.json();
...
function JournalTab() {
  ...
  async function toggleVisibility(entryId: string, currentVisible: boolean) {
    ...
    setEntries((prev) =>
      prev.map((e) =>
        e.id === entryId ? { ...e, visible_to_therapist: !currentVisible } : e,
      ),
    );
    setToggling(null);
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-foreground">
                ...
                <span className={allowAccess ? "text-success" : "text-danger"}>
                  {allowAccess ? "ON" : "OFF"}
                </span>
              </h3>
              <p className="text-xs text-muted">
        ...
        {entries.length === 0 ? (
          ...
          entries.map((entry) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  toggleVisibility(entry.id, entry.visible_to_therapist)
                }
              ...
              >
                ...
                {entry.visible_to_therapist ? "Shared" : "Hidden"}
              </button>
            </div>
          ))
        )}
      ...
      {preventive.length === 0 ? (
        ...
        <div className="space-y-3">
          {preventive.map((pm) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold text-foreground">{pm.title}</h3>
                <span className="text-xs text-muted">
...
function ReviewTab({ therapistId }: { therapistId: string }) {
  ...
  useEffect(() => {
    async function fetchReviews() {
      const res = await fetch(`/api/reviews?therapist_id=${therapistId}`, {
        cache: "no-store",
      });
      const data = await res.json();
