  ...
  useEffect(() => {
    async function fetchEntry() {
      ...
      const found = Array.isArray(data)
        ? data.find((e: JournalEntry) => e.entry_date === entryDate)
        : null;
      setEntry(found || null);
...
>>>>>>> 57e3851f669f4aed50452fdef9a3746559e43b91
  ...
  const formattedDate = new Date(entryDate + "T00:00:00").toLocaleDateString(
    "en-US",
    {
      weekday: "long",
      year: "numeric",
          ...
          {!isFuture && (
            <div className="bg-white rounded-xl border border-border p-6">
              <textarea
                value={content}
                onChange={(e) =>
                  isToday && setContent(e.target.value.slice(0, MAX_CHARS))
                }
                readOnly={!isToday}
                placeholder={
                  isToday
                ...
                className={`w-full h-96 px-4 py-3 border border-border rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors text-foreground ${
                  !isToday ? "bg-muted-bg/30 cursor-not-allowed" : ""
                }`}
              />
              <div className="flex items-center justify-between mt-3">
                <p className="text-xs text-muted">
          ...
          {entry && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl border border-border p-6">
                ...
                <p className="text-sm text-muted leading-relaxed">
                  {entry.ai_reflection ||
                    "Save your entry to get an AI reflection."}
                </p>
              </div>
